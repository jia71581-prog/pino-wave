...
class V3WavefieldDataset(Dataset[V3Record]):
    ...
    def read_wavefield(
        ...
        h5 = self._file()
        # The wavefield dataset is stored unchunked and uncompressed, and a
        # record's frames span most of the time axis (frame 60..400 of 401), so
        # neither a single fancy-index read nor a covering contiguous block is
        # cheap here: h5py's per-call overhead for a scattered index list is
        # ~68 ms regardless of how many frames it returns, while a single-frame
        # read is ~0.03 ms.  Reading the frames one at a time and stacking is
        # ~9x faster on a 12-record batch and returns bit-identical values.
        source_index = int(metadata.source_index)
        frames = np.stack(
            [h5["wavefield"][source_index, int(frame_index), :, :] for frame_index in unique]
