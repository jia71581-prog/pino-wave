...
def build_phase_features(
    ...
    *,
    ...
    # CPML boundary absorption profile (polynomial order 3, matching the data's
    # cpml_polynomial_order=3; CPML sits outside the physical domain so the model
    # sees a soft "wave leaves here" signal near the left/right/bottom edges).
    edge_left = torch.clamp((cpml_width_m - x) / cpml_width_m, 0.0, 1.0) ** 3
    edge_right = torch.clamp((cpml_width_m - (domain_x_m - x)) / cpml_width_m, 0.0, 1.0) ** 3
    edge_bottom = torch.clamp((cpml_width_m - (domain_z_m - z)) / cpml_width_m, 0.0, 1.0) ** 3
    base = torch.stack(
        (
            x / domain_x_m,
            z / domain_z_m,
...
class PhaseAlignedCoordinateEncoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        raw_features = 17 + 6 * self.fourier_bands + 2 + 2 * len(self.gabor_scales_s)
