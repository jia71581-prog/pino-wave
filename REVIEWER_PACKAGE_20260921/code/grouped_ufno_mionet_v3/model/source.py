...
class SourceEncoding:
    ...
    ic_field: torch.Tensor | None = None


class SourceEncoderV3(nn.Module):
    def __init__(self, *, width: int = 64, rank: int = 128, ic_frames: int = 0) -> None:
        super().__init__()
        if ic_frames < 0:
        ...
        if self.ic_frames:
            self.ic_encoder = nn.Sequential(
                nn.Conv2d(self.ic_frames, width, kernel_size=5, padding=2),
                nn.GELU(),
                nn.Conv2d(width, width, kernel_size=3, padding=1),
                nn.GELU(),
            )
            self.ic_projection = nn.Linear(width, width)
    ...
    def forward(
        ...
        ic_snapshots: torch.Tensor | None = None,
    ) -> SourceEncoding:
        ...
        if self.ic_frames:
            if ic_snapshots is None:
                raise ValueError("ic_snapshots is required when ic_frames is enabled")
            ic = torch.as_tensor(ic_snapshots, dtype=source.dtype, device=source.device)
            expected = (source.shape[0], self.ic_frames, *source_map.shape[-2:])
            if ic.shape != expected:
                raise ValueError(f"ic_snapshots must have shape {expected}, got {tuple(ic.shape)}")
            if not torch.isfinite(ic).all():
                raise ValueError("ic_snapshots must be finite")
        elif ic_snapshots is not None:
            raise ValueError("ic_snapshots supplied but ic_frames is disabled")
        ...
        ic_field = None
        if self.ic_frames:
            ic_field = self.ic_encoder(ic)
            local_ic = F.grid_sample(
                ic_field,
                grid,
                mode="bilinear",
                padding_mode="border",
            ...
            # 折入 map_field: 同一条空间通路直达 dense 解码器与点查询局部采样。
            map_field = map_field + ic_field
            hidden_terms = hidden_terms + self.ic_projection(local_ic)
        hidden = self.hidden_norm(hidden_terms)
    ...
    def required_gradient_groups(self) -> dict[str, tuple[nn.Parameter, ...]]:
        ...
        if self.ic_frames:
            groups["source_ic"] = tuple(
                p for n, p in named if n.startswith("ic_encoder") or n.startswith("ic_projection")
            )
        return groups
