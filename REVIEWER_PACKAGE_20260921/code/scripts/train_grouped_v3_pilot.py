...
def relative_metrics_by_family(
    ...
    *,
    ...
    counts: dict[str, int] = {}
    present = tuple(dict.fromkeys(families))  # families actually in this batch
...
def _expected_batch_balance(config: V3Config) -> dict[str, int]:
    """Expected medium count per training batch for the configured families.

    The default 3-family schedule balances 4 records per family (12 total).
    A uniform-only schedule places all 12 records in the uniform family.
    """
    families = config.data.train_families
    if set(families) == set(ALLOWED_MEDIUM_TYPES):
        return {family: 4 for family in ALLOWED_MEDIUM_TYPES}
    if families == ("uniform",):
        return {"uniform": config.train.batch_records}
    raise ValueError(
        f"pilot family balance is only supported for all-three or uniform-only; "
        f"received {families}"
    )
...
def _scratch_uniform_identity(
    config: V3Config, *, manifest_digest: str
) -> PilotIdentity:
    """Honest identity for a uniform-only, no-parent (scratch) full-data pilot.

    Records that this run is a scratch uniform training with no passed gate and
    no parent checkpoint, so a future audit can see it is not a refinement over
    a cross-family prerequisite.  The parent/prereq path fields are left empty
    and the report path is the config path (provenance only, never treated as a
    passed gate).
    """
    return PilotIdentity(
        manifest_digest=str(manifest_digest),
        gate_config_digest=str(config.digest()),
        gate_step=0,
        parent_checkpoint_path="",
        parent_checkpoint_sha256="",
        prerequisite_report_path=str(config.data.source_h5),
        prerequisite_report_sha256="uniform_scratch",
    )
...
def _resolve_pilot_identity(
    ...
    *,
    ...
    """Resolve the pilot identity, allowing a uniform-only scratch run.

    Unless ``train_families == ('uniform',)`` and no prerequisite report is
    supplied, this is byte-identical to the original
    ``validate_pilot_prerequisite``.  A uniform-only scratch run needs no passed
    cross-family gate and no parent checkpoint, so it gets a synthetic identity
    (``gate_step=0``, empty parent path) rather than a valid prerequisite.
    """
    uniform_scratch = config.data.train_families == ("uniform",)
    if uniform_scratch and prerequisite_report is None:
        return _scratch_uniform_identity(config, manifest_digest=manifest_digest)
    if initial_checkpoint is None:
        raise ValueError("pilot requires --init-checkpoint")
    return validate_pilot_prerequisite(
        prerequisite_report,
        initial_checkpoint,
...
def run_pilot(
    ...
    *,
    ...
    if config.data.train_families == ("uniform",):
        # Uniform-only: batch_records is the number of uniform records per step.
        # The 40M decoder is memory-bound, so batch 12 overflows one 24 GB card;
        # batch 6 or 8 is the practical single-card ceiling (measured 15.7/20.7 GiB).
        if config.train.batch_records < 4:
            raise ValueError("uniform-only pilot requires batch_records>=4")
    elif config.train.batch_records != 12:
    ...
    identity = _resolve_pilot_identity(
        config, prerequisite_report, initial_checkpoint, manifest_digest=manifest.digest
    )
    ...
    summary = {
        ...
        "batch_medium_count": len(set(config.data.train_families)),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--prerequisite-report", default="")
    ...
    try:
        run_pilot(
            config,
            prerequisite_report=Path(args.prerequisite_report) if args.prerequisite_report else None,
