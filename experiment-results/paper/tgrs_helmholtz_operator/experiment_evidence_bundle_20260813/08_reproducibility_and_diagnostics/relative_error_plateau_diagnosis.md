...
2. After the already queued position-only evaluation is clear of the GPUs, run the
   corrected r5e train-only ablation with the exact 75% frame-overlap contract.
3. Run equal-compute uniform-random replay as the requested random-batch ablation.
4. If train aggregate begins to fall but a later sealed validation gap appears, run
   adapter-only dropout `p=0.05` as a separate regularization ablation.
5. If corrected metric-aligned time still fails, test a frame-only loss branch before adding
   more capacity; do not combine it with sampling or dropout in the first run.
