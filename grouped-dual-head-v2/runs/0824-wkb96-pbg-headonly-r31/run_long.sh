...
artifact_dir=results/wkb96_pbg_headonly_r31_long_20260824
log_path="$artifact_dir/launch.log"
mkdir -p "$artifact_dir"
echo "[$(date -u '+%F %T UTC')] r31 P_bg head-only launch supervisor_pid=$$" >> "$log_path"
CUDA_VISIBLE_DEVICES=0 python scripts/train_wkb_frequency_train_panel.py \
  --artifact-dir "$artifact_dir" \
  --preregistration results/wkb96_pbg_headonly_r31_long_preregistration_20260824.json \
  ...
  --zero-initialize-helmholtz-head --trainable-scope helmholtz_head \
  --updates 2000 --evaluate-every 200 \
  --evaluation-frames 32 --evaluation-time-block 32 --evaluation-macro-records 1 \
  --dense-learning-rate 1e-5 --backbone-learning-rate 5e-6 --local-field-learning-rate 1e-5 \
  --family-gradient-weights marmousi:1 --target-relative-l2 0.05 --seed 372 >> "$log_path" 2>&1
status=$?
echo "[$(date -u '+%F %T UTC')] r31 P_bg head-only exit_code=$status" >> "$log_path"
exit "$status"
