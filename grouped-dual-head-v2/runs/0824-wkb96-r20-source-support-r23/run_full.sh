...
artifact_dir=results/wkb96_r20_source_support_r23_20260824
log_path="$artifact_dir/launch.log"
mkdir -p "$artifact_dir"
echo "[$(date -u '+%F %T UTC')] r23 source-support launch supervisor_pid=$$" >> "$log_path"
CUDA_VISIBLE_DEVICES=0 python scripts/train_wkb_frequency_train_panel.py \
  --artifact-dir "$artifact_dir" \
  --preregistration results/wkb96_r20_source_support_r23_preregistration_20260824.json \
...
status=$?
echo "[$(date -u '+%F %T UTC')] r23 source-support exit_code=$status" >> "$log_path"
exit "$status"
