...
artifact_dir=results/wkb96_record_freqsoftmax_r20_continue4000_20260824
log_path="$artifact_dir/launch.log"
mkdir -p "$artifact_dir"
echo "[$(date -u '+%F %T UTC')] r20 full launch supervisor_pid=$$" >> "$log_path"
CUDA_VISIBLE_DEVICES=0 python3 scripts/diagnose_capacity_ladder_overfit.py \
  ...
  --helmholtz-frequency-softmax --helmholtz-coefficient-supervision \
  ...
  --dense-learning-rate 1.0e-5 --backbone-learning-rate 5.0e-6 \
  --local-field-learning-rate 1.0e-5 \
  ...
  --split train --seed 372 >> "$log_path" 2>&1
status=$?
echo "[$(date -u '+%F %T UTC')] r20 full exit_code=$status" >> "$log_path"
exit "$status"
