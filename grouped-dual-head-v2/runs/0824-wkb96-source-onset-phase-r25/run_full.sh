...
artifact_dir=results/wkb96_source_onset_phase_r25_20260824
log_path="$artifact_dir/launch.log"
mkdir -p "$artifact_dir"
echo "[$(date -u '+%F %T UTC')] r25 onset-phase launch supervisor_pid=$$" >> "$log_path"
CUDA_VISIBLE_DEVICES=0 python scripts/train_wkb_frequency_train_panel.py \
  --artifact-dir "$artifact_dir" \
  --preregistration results/wkb96_source_onset_phase_r25_preregistration_20260824.json \
...
status=$?
echo "[$(date -u '+%F %T UTC')] r25 onset-phase exit_code=$status" >> "$log_path"
exit "$status"
