#!/usr/bin/env python3
"""Evaluate Phase4b on one sealed fixed-19-Hz Marmousi position panel.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="input-only Marmousi velocity-complexity rank from the registered protocol",
    )
...
def main() -> int:
    ...
    rank = int(args.rank)
    if not 1 <= rank <= 30:
        raise ValueError("--rank must lie in the registered range 1..30")
    ...
    if sha256_file(velocity_path) != input_row["input_file_sha256"]:
        raise ValueError(f"rank-{rank} input file hash changed")
    ...
    if _array_sha256(velocity_mps) != input_row["velocity_array_sha256"]:
        raise ValueError(f"rank-{rank} velocity bytes changed")
    if velocity_mps.shape != (201, 201) or time_s.shape != (401,):
        raise ValueError(f"rank-{rank} input geometry changed")
    ...
    if set(sealed_rows) != set(CASE_ORDER):
        raise ValueError(f"rank-{rank} sealed cases differ from the eight registered positions")
    ...
    prediction_manifest: dict[str, Any] = {
        "schema": "phase4b_fixed19hz_marmousi_multisource_prediction_seal_v1",
        "status": "building",
    ...
    for number, item in enumerate(case_inputs, start=1):
        ...
        prediction_path = output_dir / "sealed_predictions" / f"marm_r{rank:02d}_{item['case_id']}.npz"
        _atomic_npz(
            prediction_path,
    ...
    if set(reference_rows) != set(CASE_ORDER):
        raise ValueError(f"rank-{rank} reference cases changed")
    ...
    report = {
        "schema": "phase4b_fixed19hz_marmousi_multisource_evaluation_v1",
        "status": "complete",
        "claim_scope": {
            "varied_variable": "source_position_only",
