#!/usr/bin/env python3
"""Gate a 101-mode pilot against its exact fixed-panel parent evidence."""
...
def _best_epoch(path: Path) -> dict[str, object]:
    ...
    if not epochs:
        raise ValueError("all-mode metrics contain no fixed-panel epoch")
    return min(
        epochs,
        key=lambda row: (
...
def main(argv=None) -> int:
    ...
    report.update(
        {
            "schema": "saved_time_all_modes_evidence_gate_v1",
