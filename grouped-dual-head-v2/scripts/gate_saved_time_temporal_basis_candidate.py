#!/usr/bin/env python3
"""Gate a temporal-basis pilot against its exact corrected parent."""
...
from saved_time_phase_operator_v4.continuation import temporal_basis_candidate_gate
...
def _best_epoch(path: Path) -> dict[str, object]:
    ...
    if not epochs:
        raise ValueError("temporal-basis metrics contain no fixed-panel epoch")
    return min(
        epochs,
        key=lambda row: (
...
def main(argv=None) -> int:
    ...
    report.update(
        {
            "schema": "saved_time_temporal_basis_evidence_gate_v1",
            ...
            "temporal_basis_rank": int(selection["temporal_basis_rank"]),
        }
    )
