#!/usr/bin/env python3
"""Gate a temporal-basis delta-floor continuation against its exact parent."""
...
def _best_epoch(path: Path) -> dict[str, object]:
    ...
    if not epochs:
        raise ValueError("delta-floor metrics contain no fixed-panel epoch")
    return min(
        epochs,
        key=lambda row: (
...
def _completed_optimizer_updates(path: Path) -> int:
    ...
    try:
        ...
        raise ValueError("delta-floor epoch is missing optimizer progress") from error


def _intervention_metadata(selection: dict[str, object]) -> dict[str, object]:
    """Preserve the metadata emitted by either temporal continuation builder."""

    if "dense_learning_rate_multiplier" in selection:
        return {
            "dense_learning_rate_multiplier": float(
                selection["dense_learning_rate_multiplier"]
            )
        }
    if "dense_learning_rate" in selection:
        return {
            "dense_learning_rate": float(selection["dense_learning_rate"]),
            "gate_absorption": bool(selection.get("gate_absorption", False)),
        }
    raise ValueError("temporal selection is missing dense learning-rate metadata")
...
def main(argv=None) -> int:
    ...
    candidate = _best_epoch(metrics_path)
    gate = config["gate"]
    report = temporal_delta_floor_candidate_gate(
        ...
        completed_optimizer_updates=_completed_optimizer_updates(metrics_path),
    ...
    report.update(
        {
            "schema": "saved_time_temporal_delta_floor_evidence_gate_v1",
            "selection_report": str(selection_path.resolve()),
            "candidate_metrics": str(metrics_path.resolve()),
            "candidate_epoch": int(candidate["epoch"]),
            "candidate_checkpoint": str(candidate["checkpoint"]),
            "delta_energy_floor_fraction": float(
                selection["delta_energy_floor_fraction"]
            ),
            **_intervention_metadata(selection),
        }
    )
