#!/usr/bin/env python3
"""Materialize an exact temporal-gate reparameterization candidate."""
...
def main(argv=None) -> int:
    ...
    evidence = {
        "schema": "saved_time_temporal_gate_absorption_parent_selection_v1",
        "name": selection.name,
        ...
        "temporal_basis_rank": int(config["variant_overrides"]["temporal_basis_rank"]),
        ...
        * int(config["macros_per_update"]),
        ...
        "delta_energy_floor_fraction": float(
            config["loss"]["delta_energy_floor_fraction"]
        ),
