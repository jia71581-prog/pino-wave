#!/usr/bin/env python3
"""Freeze fresh train-only groups for the coupled mHC/Muon factorial pilot."""
...
EXCLUDE = ROOT / "results/transfer_dg_phase_scatter64_pilot_manifest_20260903.json"
OUTPUT = ROOT / "results/transfer_dg_coupled_mhc_muon_pilot_manifest_20260903.json"
...
def order(group: str) -> str:
    return hashlib.sha256(f"coupled-mhc-muon-v1:{group}".encode()).hexdigest()
...
def main() -> int:
    ...
    previous = json.loads(EXCLUDE.read_text())
    excluded_groups = {row["group_id"] for row in previous["records"]}
    ...
    overlaps = sum(len(role_groups[a] & role_groups[b]) for a, b in (
        ("fit", "calibration"), ("fit", "confirmation"),
        ("calibration", "confirmation")
    ))
    if overlaps or any(row["group_id"] in excluded_groups for row in selected):
        raise RuntimeError("fresh factorial split overlaps prior evidence")
    payload = {
        "schema": "transfer_dg_coupled_mhc_muon_pilot_manifest_v1",
        "split": "train",
        "source_h5": source["source_h5"],
        "record_count": len(selected),
        "role_counts": {role: sum(row["role"] == role for row in selected)
                        for role in ("fit", "calibration", "confirmation")},
        ...
        "selection_policy": "hash-order unused parent-fit groups with fixed per-family group counts",
        "records": selected,
    ...
    print(json.dumps({key: payload[key] for key in (
        ...
        "role_group_overlap_count", "excluded_prior_group_count",
        "selection_sha256", "validation_opened", "test_id_opened"
