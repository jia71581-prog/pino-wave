#!/bin/bash
# Detached Stage-B GPU pilot for the reduced CHONKNORIS research model.
set -Eeuo pipefail
...
RUN_DIR="$ARTIFACT_ROOT/gpu_pilot_20260807T0919CST"
OUTPUT_CKPT="$ARTIFACT_ROOT/meta_hypernet_chonknoris.pt"
...
finish_run() {
  ...
  echo "[chonknoris] status=$status_value rc=$rc finished_at=$finished_at elapsed_s=$elapsed_seconds"
  exit "$rc"
}
...
echo "[chonknoris] started_at=$STARTED_AT"
echo "[chonknoris] run_dir=$RUN_DIR"
echo "[chonknoris] output=$OUTPUT_CKPT"
