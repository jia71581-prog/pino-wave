...
INIT="$ROOT/results/r38_full_coverage_finetune_1e5_5x1000_v1_20260828/best.pt"
OUT="$ROOT/results/r39_hfs_tail_finetune_base1e6_hfs1e4_4x500_v3_20260828"
TORCHRUN="/root/miniconda3/bin/torchrun"

cd "$ROOT"
if [[ -e "$OUT" ]]; then
  echo "refusing to reuse existing R39 directory: $OUT" >&2
  exit 2
fi
test -s "$INIT"
