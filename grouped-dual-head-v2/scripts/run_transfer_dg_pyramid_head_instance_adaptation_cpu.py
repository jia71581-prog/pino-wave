#!/usr/bin/env python3
"""Run two-onset Transfer-DG adaptation in a physical-head channel span."""
...
from scripts.diagnose_transfer_dg_pyramid_head_basis_oracle_cpu import (  # noqa: E402
    predict_and_build_head_basis,
)
...
def main() -> int:
    ...
    observed_bindings = {
        ...
        "head_basis_helper_sha256": sha256(
            ROOT / "scripts/diagnose_transfer_dg_pyramid_head_basis_oracle_cpu.py"
        ),
        "integration_common_sha256": sha256(
    ...
    try:
        ...
        parent_coefficients, raw_basis = predict_and_build_head_basis(model, public)
        del model
        gc.collect()
        channel_norms = raw_basis.reshape(raw_basis.shape[0], -1).norm(dim=1)
        top_k = int(prereg["adaptation"]["top_energy_channels"])
        ...
        del raw_basis
        gc.collect()
        frequency_hz = torch.from_numpy(public.frequency_hz).float()
