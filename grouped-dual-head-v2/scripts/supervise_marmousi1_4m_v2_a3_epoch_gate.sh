#!/usr/bin/env bash
# Resume the accepted checkpoint on four GPUs with bounded host page cache.
set -Eeuo pipefail
...
resume_log="$artifact_dir/four_gpu_cache_bounded_resume.log"

mkdir -p "$artifact_dir"
exec 9>"$artifact_dir/four_gpu_cache_bounded.lock"
if ! flock -n 9; then
  echo "another cache-bounded recovery supervisor owns the lock" >&2
  exit 73
fi
cd "$project_root"
...
on_exit() {
  local rc=$?
  if [[ "$rc" -ne 0 ]]; then
    write_terminal failed cache_bounded_recovery "$rc" || true
  fi
}
trap on_exit EXIT
...
PY
  ...
  if [[ "$external_attempt" -ge "$maximum_external_attempts" ]]; then
    echo "external recovery attempts exhausted" >&2
    exit 75
  fi
...
write_terminal running reproducibility_verification 0
bash scripts/run_marmousi1_4m_v2_a3_warm_full_resume.sh >>"$resume_log" 2>&1
