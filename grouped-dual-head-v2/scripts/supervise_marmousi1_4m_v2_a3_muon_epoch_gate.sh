#!/usr/bin/env bash
# Continue the accepted A3 optimum on four GPUs with Muon + auxiliary AdamW.
set -Eeuo pipefail

project_root=/root/autodl-tmp/work/FNO-Acoustic-Wave-Simulation/grouped-dual-head-v2
config=configs/saved_time_v4/generated/local_field_w128_temporal_latent_a3_rank32_warm_marmousi1_4m_v2_r4_muon.yaml
artifact_dir=/root/autodl-tmp/home/jiayh/Data/FNO-Acoustic-Wave-Simulation/1/pretraining/local_field_w128_hicap/temporal_latent_a3_rank32_warm_marmousi1_4m_v2_r4_muon
...
resume_log="$artifact_dir/four_gpu_muon_resume.log"

mkdir -p "$artifact_dir"
printf '%s\n' "$$" >"$artifact_dir/four_gpu_muon_supervisor.pid"
exec 9>"$artifact_dir/four_gpu_muon.lock"
if ! flock -n 9; then
  echo "another Muon continuation supervisor owns the lock" >&2
  exit 73
fi
cd "$project_root"
...
on_exit() {
  local rc=$?
  if [[ "$rc" -ne 0 ]]; then
    write_terminal failed muon_recovery "$rc" || true
  fi
}
trap on_exit EXIT
...
PY
  ...
  write_terminal running "four_gpu_muon_attempt_${external_attempt}" 0
  set +e
