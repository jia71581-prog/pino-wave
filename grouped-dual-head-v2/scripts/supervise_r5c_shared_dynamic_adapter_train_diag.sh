#!/usr/bin/env bash
# One-shot smoke + four-GPU diagnostic continuation. No restart or process signals.
set -Eeuo pipefail

project_root=/root/autodl-tmp/work/FNO-Acoustic-Wave-Simulation/grouped-dual-head-v2
config=configs/saved_time_v4/generated/local_field_w128_temporal_latent_a3_rank32_r5c_shared_dynamic_adapter_train_diag.yaml
artifact_dir=/root/autodl-tmp/home/jiayh/Data/FNO-Acoustic-Wave-Simulation/1/pretraining/local_field_w128_hicap/temporal_latent_a3_rank32_r5c_shared_dynamic_adapter_train_diag
...
smoke_log_path="$artifact_dir/four_gpu_smoke.log"

mkdir -p "$artifact_dir"
printf '%s\n' "$$" >"$artifact_dir/supervisor.pid"
exec 9>"$artifact_dir/supervisor.lock"
if ! flock -n 9; then
  printf '%s\n' "another r5c supervisor owns the lock" >&2
  exit 73
fi
cd "$project_root"
...
payload = {
    "schema": "r5c_shared_dynamic_adapter_supervisor_v1",
    "status": sys.argv[2],
    "stage": sys.argv[3],
...
if [[ -e "$artifact_dir/run/run_identity.json" ]]; then
  printf '%s\n' "r5c artifact already has a run identity; refusing overwrite" >&2
  exit 74
fi
...
write_terminal running four_gpu_smoke 0
set +e
"$torchrun_bin" --standalone --nproc_per_node=4 \
  scripts/train_saved_time_v4_full_support.py \
  --config "$config" --smoke-updates 2 --workers-override 0 \
  >>"$smoke_log_path" 2>&1
smoke_rc=$?
set -e
if [[ "$smoke_rc" -ne 0 ]]; then
  write_terminal failed smoke "$smoke_rc"
  trap - EXIT
  exit "$smoke_rc"
fi

"$python_bin" - "$artifact_dir/smoke/terminal.json" <<'PY'
import json, pathlib, sys
terminal = json.loads(pathlib.Path(sys.argv[1]).read_text())
if terminal.get("status") != "complete" or terminal.get("mode") != "smoke":
    raise SystemExit("smoke terminal is not complete")
PY
