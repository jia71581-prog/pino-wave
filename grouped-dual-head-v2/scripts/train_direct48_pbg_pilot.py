#!/usr/bin/env python3
"""r49 ``direct48_pbg`` -- minimal project-train 3-record overfit capacity pilot.

Scientific contract (audit: cc_mapper_bottleneck_audit_prompt, capacity basis:
r48b ``trainonly_spectral_capacity``):
  ...
  * Model inputs are velocity, source_map/source_parameters, coordinates,
    ...
    model input.
  * The direct-frequency head (helmholtz_frequencies=48, rank=0, wkb=false)
    outputs [B, 96, 64, 64] and the fixed inverse synthesis renders
    [B, 401, 64, 64].  The target follows r48b: the loader's bilinear+antialias
    64x64 spatial sampling over all 401 stored time points, encode_pressure,
    then a forward-norm rFFT over bins 0..47 with cos=2Re / sin=-2Im and the DC
    special case.  Global pressure_scale_pa=1.894229157173348e-08 and
    amplitude=1; no per-record or per-frequency target normalization.
  * Training follows r4: AdamW dense 1e-4 / backbone 5e-5, seed 372, 800
    ...
    IS the 48-bin inverse synthesis.
...
_SCHEMA = "direct48_pbg_pilot_v1"
_TERMINAL_SCHEMA = "direct48_pbg_pilot_terminal_v1"
...
def validate_run_config(config: Mapping[str, object]) -> None:
    if str(config.get("schema")) != "direct48_pbg_pilot_run_v1":
    ...
    if bool(helmholtz.get("wkb_phase", True)):
        raise ValueError("direct48 pilot requires wkb_phase=false")
    if int(helmholtz.get("rank", -1)) != 0:
        raise ValueError("direct48 pilot requires the independent head (rank=0)")
...
def main(argv=None) -> int:
    ...
    variant = ProbeVariant(
        ...
        local_field_helmholtz_synthesis_wkb_phase=False,
    ...
    if int(head.out_channels) != 2 * int(helmholtz["frequencies"]):
        raise RuntimeError("helmholtz head channel count changed")
    if head.weight.shape[-2:] != (1, 1):
        raise RuntimeError("helmholtz head is not the independent 1x1 coefficient head")
