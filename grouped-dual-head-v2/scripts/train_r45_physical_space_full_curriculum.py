#!/usr/bin/env python3
"""R45 full-fit physical-space residual FNO with hard-record curriculum.
...
def main() -> int:
    ...
    if min(
        ...
        raise ValueError("R45 count arguments must be positive")
    ...
    r40 = load_module(r40_path, "r45_r40")
    r42 = load_module(r42_path, "r45_r42")
    r44 = load_module(r44_path, "r45_r44")
    fit = None
    holdout = None
    try:
        ...
        if fit.selection_sha256 != holdout.selection_sha256:
            raise RuntimeError("R45 fit/holdout selection mismatch")
        if set(fit.group_ids) & set(holdout.group_ids):
            raise RuntimeError("R45 fit/holdout group leakage")
        if int(fit.retained) != int(holdout.retained):
            raise RuntimeError("R45 fit/holdout DCT geometry mismatch")
        if not np.array_equal(fit.frequency_indices, holdout.frequency_indices):
            raise RuntimeError("R45 fit/holdout frequency mismatch")

        curriculum_path = args.curriculum_manifest.expanduser().resolve()
        curriculum, sampling_weights, base_errors = r42.validate_curriculum(
            curriculum_path, fit
        )
        manifest_records = list(curriculum["records"])
        output_dir = args.output_dir.expanduser().resolve()
        if output_dir.exists() and any(output_dir.iterdir()):
            raise FileExistsError(f"refusing nonempty R45 output: {output_dir}")
        if rank == 0:
        ...
        identity = {
            "schema": "r45_physical_space_full_curriculum_identity_v1",
        ...
        if rank == 0:
            ...
            if float(initial["correction_scale"]) != 0.0:
                raise RuntimeError("R45 zero model did not select identity")
            ...
            if identity_difference > 1.0e-8:
                raise RuntimeError("R45 zero model changed the parent metric")
            ...
            checkpoint = {
                "schema": "r45_physical_space_full_curriculum_checkpoint_v1",
                "epoch": 0,
                "global_step": 0,
        ...
        for epoch in range(1, int(args.epochs) + 1):
            ...
            for batch in loader:
                ...
                if not torch.isfinite(loss):
                    raise FloatingPointError(
                        f"nonfinite R45 loss at rank {rank}, step {global_step + 1}"
                    )
                loss.backward()
            ...
            if epoch_records != int(args.records_per_rank_epoch):
                raise RuntimeError("R45 epoch record count mismatch")
            scheduler.step()
        ...
        if rank == 0:
            summary = {
                "schema": "r45_physical_space_full_curriculum_summary_v1",
                "status": "complete",
