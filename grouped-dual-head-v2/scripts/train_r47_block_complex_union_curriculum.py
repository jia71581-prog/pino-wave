#!/usr/bin/env python3
"""R47 low-dimensional block complex modulation on the R46 fit union.
...
def block_gain_target(
    base_spatial: torch.Tensor,
    ...
    *,
    ...
    base = base_spatial.float()
    residual = residual_spatial.float()
    base_real, base_imag = base[:, 0], base[:, 1]
    residual_real, residual_imag = residual[:, 0], residual[:, 1]
    numerator_real = base_real * residual_real + base_imag * residual_imag
    numerator_imag = base_real * residual_imag - base_imag * residual_real
    denominator = base_real.square() + base_imag.square()
    pooled_denominator = partition_pool(
        denominator[:, None],
        (block_grid, block_grid),
        reduction="sum",
    )[:, 0]
    pooled_real = partition_pool(
        numerator_real[:, None],
        (block_grid, block_grid),
        reduction="sum",
    )[:, 0]
    pooled_imag = partition_pool(
        numerator_imag[:, None],
        (block_grid, block_grid),
        reduction="sum",
    )[:, 0]
    safe_denominator = pooled_denominator.clamp_min(1.0e-8)
    target = torch.stack(
        (pooled_real / safe_denominator, pooled_imag / safe_denominator), dim=1
    ).clamp(-float(gain_cap), float(gain_cap))
    energy_weight = pooled_denominator / pooled_denominator.mean(
        dim=(1, 2), keepdim=True
    ).clamp_min(1.0e-8)
    return target.detach(), energy_weight.clamp(0.0, 10.0).detach()
...
def main() -> int:
    ...
    parser.add_argument("--r47-model-script", type=Path, required=True)
    ...
    parser.add_argument("--encoder-width", type=int, default=96)
    parser.add_argument("--low-resolution-blocks", type=int, default=4)
    parser.add_argument("--gain-cap", type=float, default=0.5)
    parser.add_argument("--gain-loss-weight", type=float, default=0.05)
    ...
    if min(
        ...
        raise ValueError("R47 count arguments must be positive")
    if (
        int(args.block_grid) < 2
        or int(args.encoder_width) < 16
        or int(args.low_resolution_blocks) < 1
        or float(args.gain_cap) <= 0.0
        or float(args.gain_loss_weight) < 0.0
    ):
        raise ValueError("invalid R47 block-modulation configuration")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("R47 requires CUDA")
    ...
    model_path = args.r47_model_script.expanduser().resolve()
    r40 = load_module(r40_path, "r47_r40")
    r42 = load_module(r42_path, "r47_r42")
    r44 = load_module(r44_path, "r47_r44")
    r46_union = load_module(union_path, "r47_union")
    r47_model = load_module(model_path, "r47_model")
    fit = None
    holdout = None
    try:
        ...
        if str(fit.component_selection_sha256[0]) != str(
            ...
            raise RuntimeError("R47 base-fit/holdout source selection mismatch")
        if set(fit.group_ids) & set(holdout.group_ids):
            raise RuntimeError("R47 fit/holdout group leakage")
        if int(fit.retained) != int(holdout.retained):
            raise RuntimeError("R47 fit/holdout DCT geometry mismatch")
        if not np.array_equal(fit.frequency_indices, holdout.frequency_indices):
            raise RuntimeError("R47 fit/holdout frequency mismatch")
        ...
        if output_dir.exists() and any(output_dir.iterdir()):
            raise FileExistsError(f"refusing nonempty R47 output: {output_dir}")
        if rank == 0:
        ...
        model_for_save = r47_model.BlockComplexGainRefiner(
            block_grid=int(args.block_grid),
            encoder_width=int(args.encoder_width),
            gain_cap=float(args.gain_cap),
            low_resolution_blocks=int(args.low_resolution_blocks),
        ).to(device)
        ...
        identity = {
            "schema": "r47_block_complex_union_curriculum_identity_v1",
            ...
            "model": {
                "name": "low_dimensional_block_complex_gain_refiner",
                "parameter_count": r47_model.parameter_count(model_for_save),
                "block_grid": int(args.block_grid),
                "encoder_width": int(args.encoder_width),
                "low_resolution_blocks": int(args.low_resolution_blocks),
                "gain_cap": float(args.gain_cap),
                "upsampling": "nearest_piecewise_constant",
                "partition_pooling": "nearest_aligned_nonoverlapping",
                "correction_parameterization": "complex_gain_times_parent_wavefield",
            ...
            "optimization": {
                "objective": (
                    "sampled_full_wavefield_relative_energy_with_tail_hinge_"
                    "plus_energy_weighted_block_gain_supervision"
                ),
            ...
            "evidence": {
                ...
                "r47_model_script": str(model_path),
                "r47_model_script_sha256": r42.sha256_file(model_path),
        ...
        if rank == 0:
            ...
            if float(initial["correction_scale"]) != 0.0:
                raise RuntimeError("R47 zero model did not select identity")
            ...
            if identity_difference > 1.0e-8:
                raise RuntimeError("R47 zero model changed the parent metric")
            ...
            checkpoint = {
                "schema": "r47_block_complex_union_curriculum_checkpoint_v1",
                "epoch": 0,
                "global_step": 0,
        ...
        for epoch in range(1, int(args.epochs) + 1):
            ...
            for batch in loader:
                ...
                with amp_context:
                    prediction, predicted_gain = model(
                        features, return_gain=True
                    )
                target_gain, gain_energy_weight = block_gain_target(
                    features[:, :2],
                    target_spatial,
                    block_grid=int(args.block_grid),
                    gain_cap=float(args.gain_cap),
                    partition_pool=r47_model.nearest_aligned_partition_pool2d,
                )
                error_square = (
                ...
                gain_loss = (
                    gain_error * gain_energy_weight
                ).sum() / gain_energy_weight.sum().clamp_min(1.0e-8)
                loss = (
                    physical_mean
                    + float(args.tail_weight) * physical_tail
                    + float(args.hinge_weight) * hinge
                    + float(args.shape_weight) * shape
                    + float(args.gain_loss_weight) * gain_loss
                )
                if not torch.isfinite(loss):
                    raise FloatingPointError(
                        f"nonfinite R47 loss at rank {rank}, step {global_step + 1}"
                    )
                loss.backward()
            ...
            if epoch_records != int(args.records_per_rank_epoch):
                raise RuntimeError("R47 epoch record count mismatch")
            scheduler.step()
        ...
        if rank == 0:
            summary = {
                "schema": "r47_block_complex_union_curriculum_summary_v1",
                "status": "complete",
