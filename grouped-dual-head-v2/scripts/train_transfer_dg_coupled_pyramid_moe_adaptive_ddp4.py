#!/usr/bin/env python3
"""Resume pyramid-MoE DDP pretraining with bounded train-only feedback control."""
...
def main() -> int:
    ...
    if start_epoch < int(prereg["handoff"]["expected_start_epoch"]):
    ...
    if resume.get("schema") == "transfer_dg_coupled_pyramid_moe_adaptive_ddp4_checkpoint_v1":
        ...
        if start_step != 0 or start_epoch != int(prereg["handoff"]["expected_start_epoch"]):
            raise RuntimeError("initial fixed-to-adaptive handoff must occur exactly at an epoch boundary")
    ...
    identity = {
        "schema": "transfer_dg_coupled_pyramid_moe_adaptive_ddp4_identity_v1",
        "world_size": world,
        "model": "PyramidMoECoupledWaveOperator",
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        if is_rank0:
            atomic_checkpoint(
                checkpoint_payload(
                    model, optimizer, identity, controller_state,
                    epoch=epoch + (0 if stopped_early else 1), next_step=0, update=update,
                ),
                latest_path,
            )
            with metrics_path.open("a") as handle:
