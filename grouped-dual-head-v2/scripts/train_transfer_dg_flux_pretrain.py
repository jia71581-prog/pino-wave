#!/usr/bin/env python3
"""Transfer DG pretraining: paired conservative interface-flux continuation."""
...
def weighted_multiphase_score(phases: list[dict]) -> float:
    if len(phases) != 3:
        raise ValueError("Transfer DG pretraining requires three phase metrics")
    return float(
        sum(weight * float(row["aggregate"]) for weight, row in zip(SLOT_WEIGHTS, phases))
    )


def dg_rank_for_arm(arm: str) -> int:
    if arm == "dg_flux":
        return 16
    if arm == "control":
        return 0
    raise ValueError(f"unknown Transfer DG pretraining arm: {arm}")


def _model(*, dg_rank: int, device: torch.device) -> FrameConditionedPropagator:
    model = FrameConditionedPropagator(
        ...
        dg_interface_rank=int(dg_rank),
        dg_cpml_margin=20,
    ).to(device)
    # The outer per-time-step checkpoint already discards the complete step
    # graph.  Checkpointing every spectral block inside it only recomputes the
    # same graph twice and measured 20% slower on the target GPU.
    model.step_stack.activation_checkpointing = False
    return model


def _verify(paths: list[Path], expected: dict[str, str], label: str) -> None:
    for path in paths:
...
def main() -> int:
    ...
    parser.add_argument("--arm", choices=("dg_flux", "control"), required=True)
    ...
    if len(args.fit_manifest) != 3 or len(args.fit_cache) != 3:
        raise ValueError("three fit windows are required")
    if len(args.holdout_manifest) != 3 or len(args.holdout_cache) != 3:
        raise ValueError("three holdout windows are required")
    ...
    try:
        ...
        if prereg["method"] != "Transfer DG" or prereg["stage"] != "pretrain_DG_flux_v1":
            raise RuntimeError("wrong Transfer DG pretraining preregistration")
        ...
        _verify(args.fit_manifest, bindings["fit_manifest_sha256"], "fit manifest")
        _verify(args.fit_cache, bindings["fit_cache_sha256"], "fit cache")
        _verify(
            args.holdout_manifest,
            bindings["holdout_manifest_sha256"],
            "holdout manifest",
        )
        _verify(args.holdout_cache, bindings["holdout_cache_sha256"], "holdout cache")
        ...
        for manifest in fit_manifests + holdout_manifests:
            if (
                ...
                raise RuntimeError("sealed train manifests are required")
        ...
        parent = _model(dg_rank=0, device=device)
        ...
        torch.manual_seed(args.seed + 503)
        dg_rank = dg_rank_for_arm(args.arm)
        model = _model(dg_rank=dg_rank, device=device)
        incompatible = model.load_state_dict(checkpoint["model_state"], strict=False)
        if incompatible.unexpected_keys:
            raise RuntimeError(f"unexpected parent keys: {incompatible.unexpected_keys}")
        if args.arm == "control" and incompatible.missing_keys:
            raise RuntimeError(f"control load is not strict: {incompatible.missing_keys}")
        if args.arm == "dg_flux" and (
            not incompatible.missing_keys
            or not all(key.startswith("dg_interface.") for key in incompatible.missing_keys)
        ):
            raise RuntimeError(f"invalid DG transfer keys: {incompatible.missing_keys}")
        ...
        if initial_score != parent_score:
            raise RuntimeError(
                f"zero-gated DG transfer changed the parent score: {initial_score} != {parent_score}"
            )

        fit = ThreeWindowCache(args.fit_cache, fit_manifests)
        torch.manual_seed(args.seed)
        rng = np.random.default_rng(args.seed + 607)
        ...
        identity = {
            "schema": "transfer_dg_flux_pretrain_lane_identity_v1",
            "method": "Transfer DG",
            "stage": "pretrain_DG_flux_v1",
            "arm": args.arm,
            "seed": args.seed,
            ...
            "dg_interface_rank": dg_rank,
            "dg_cpml_margin": 20,
            "checkpointing": "outer_time_step_only",
        ...
        _atomic_json(
            {
                ...
                "bit_exact_metric_transfer": True,
            },
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for _ in range(args.steps_per_epoch):
                ...
                if not torch.isfinite(loss):
                    raise FloatingPointError("nonfinite Transfer DG pretraining loss")
                loss.backward()
            ...
            dg_scale = (
                0.0
                if model.dg_interface is None
                else float(torch.tanh(model.dg_interface.scale.detach()))
            )
            event = {
                "event": "epoch",
                "arm": args.arm,
