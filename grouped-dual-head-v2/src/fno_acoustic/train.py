...
from .data import PinoHDF5Dataset, collate_pino, create_splits, sample_count_from_config
from .losses import combined_loss

# Optional physics/receiver/spectral loss terms. These submodules are not present in every
# checkout; they are only needed when the corresponding loss weight is non-zero (all default
# to 0.0). Import lazily so the supervised-only training path works without them, and fail
# with a clear message only if a missing term is actually requested.
def _missing_loss(module_name: str, symbol: str):
    def _raise(*_args, **_kwargs):
        raise ImportError(
            f"{symbol} requires fno_acoustic.{module_name}, which is not present in this "
            f"checkout; set the corresponding loss weight to 0 or restore the module."
        )
    return _raise


try:
    from .losses_physics import energy_stability_loss, pde_residual_loss
except ImportError:
    energy_stability_loss = _missing_loss("losses_physics", "energy_stability_loss")
    pde_residual_loss = _missing_loss("losses_physics", "pde_residual_loss")
try:
    from .losses_receiver import receiver_waveform_loss
except ImportError:
    receiver_waveform_loss = _missing_loss("losses_receiver", "receiver_waveform_loss")
try:
    from .losses_spectral import spectral_band_loss
except ImportError:
    spectral_band_loss = _missing_loss("losses_spectral", "spectral_band_loss")
from .metrics import basic_metrics
from .model import AcousticFNO3D, build_model_config
...
def _load_or_compute_normalization(config: dict[str, Any], train_indices: list[int], write: bool = True) -> dict[str, Any]:
...
def _ddp_enabled(config: dict[str, Any]) -> bool:
    return bool(config.get("train", {}).get("ddp", False))


def _ddp_rank_world_size() -> tuple[int, int]:
    return int(os.environ.get("RANK", 0)), int(os.environ.get("WORLD_SIZE", 1))


def _allreduce_grads_complex_safe(model: torch.nn.Module, ddp_world: int) -> None:
    # NCCL does not support complex dtypes in torch 2.1, and the spectral
    # mixer holds ComplexFloat weights, so DDP's own reduce paths cannot be
    # used.  Sync gradients manually by viewing complex grads as real pairs
    # and all-reducing one flat float tensor (AVG, matching DDP semantics).
    # Identical init (shared seed) plus identical reduced grads keep all
    # ranks bit-for-bit in sync without a parameter broadcast.
    flat_parts: list[torch.Tensor] = []
    for param in model.parameters():
        if param.grad is None:
            continue
        grad = param.grad
        flat_parts.append((torch.view_as_real(grad) if grad.is_complex() else grad).contiguous().view(-1))
    if not flat_parts:
        return
    ...
    for param in model.parameters():
        ...
        numel = grad.numel()
        if grad.is_complex():
            synced = flat[offset : offset + 2 * numel].view_as(torch.view_as_real(grad))
            grad.copy_(torch.view_as_complex(synced))
            offset += 2 * numel
        else:
...
def validate(
    ...
    summary = {k: float(weighted_sums[k] / total_samples) for k in collected[0]}
    if zero_collected:
    ...
    if ddp_reduce and dist.is_initialized():
        world = int(os.environ.get("WORLD_SIZE", 1))
        metric_keys = sorted(weighted_sums)
        reduce_values = [weighted_sums[k] for k in metric_keys] + [float(total_samples)]
        tensor = torch.tensor(reduce_values, dtype=torch.float64, device=device)
        dist.all_reduce(tensor, op=dist.ReduceOp.SUM)
        values = tensor.tolist()
        merged_sums = dict(zip(metric_keys, values[:-1]))
        merged_total = int(values[-1])
        summary = {k: float(merged_sums[k] / merged_total) for k in metric_keys}
        if zero_collected:
            local_zero = float(torch.cat(zero_collected).mean())
            zero_tensor = torch.tensor([local_zero * total_samples, float(total_samples)], dtype=torch.float64, device=device)
            dist.all_reduce(zero_tensor, op=dist.ReduceOp.SUM)
            summary["zero_baseline_relative_l2"] = float(zero_tensor[0] / zero_tensor[1])
        gathered = [None] * world
        dist.all_gather_object(gathered, (category_sums, category_counts))
        merged_category_sums: dict[str, float] = {}
        merged_category_counts: dict[str, int] = {}
        for rank_sums, rank_counts in gathered:
            for category, value in (rank_sums or {}).items():
                merged_category_sums[category] = merged_category_sums.get(category, 0.0) + float(value)
            for category, value in (rank_counts or {}).items():
                merged_category_counts[category] = merged_category_counts.get(category, 0) + int(value)
        for category in sorted(merged_category_sums):
            count = max(1, merged_category_counts.get(category, 0))
            summary[f"relative_l2_by_category/{category}"] = float(merged_category_sums[category] / count)
            summary[f"sample_count_by_category/{category}"] = float(merged_category_counts.get(category, 0))
    return summary
...
def estimate_memory(config: dict[str, Any]) -> dict[str, Any]:
    ...
    model_name = str(model_cfg.get("name", "acoustic_fno")).lower()
...
def _spatial_residual_map(pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    if pred.ndim != 4 or target.ndim != 4:
        raise ValueError("spatial residual sampling expects pred and target shaped [batch, x, z, time]")
    ...
    return residual.mean(dim=(0, 3)).cpu()
...
def _spatial_importance_loss(
    ...
    flat_pred = pred.reshape(batch, height * width, time_steps)
    flat_target = target.reshape(batch, height * width, time_steps)
    ...
    if not reweight:
        ...
        return combined_loss(selected_pred, selected_target, **sampled_loss_config)

    probabilities = flat_probabilities.to(device=pred.device, dtype=pred.dtype).clamp_min(1.0e-12)
    weights = (1.0 / (float(full_spatial_count) * probabilities)).view(1, -1, 1)
    diff = selected_pred - selected_target
    weighted_diff2 = weights * diff.pow(2)
    weighted_target2 = weights * selected_target.pow(2)
    numerator = torch.sqrt(weighted_diff2.sum(dim=(1, 2)).clamp_min(0.0))
    denominator = torch.clamp(
        torch.sqrt(weighted_target2.sum(dim=(1, 2)).clamp_min(0.0)),
...
PHYSICS_LOSS_KEYS = {"pde_weight", "energy_weight", "receiver_weight", "komega_weight"}
...
def _source_term_from_batch(batch: dict[str, Any], device: torch.device, dtype: torch.dtype) -> torch.Tensor:
    ...
    if wavelet is None:
        wavelet = torch.zeros(source_map.shape[0], batch["target"].shape[-1], device=device, dtype=dtype)
    else:
        wavelet = wavelet.to(device=device, dtype=dtype)
    return source_map.unsqueeze(-1) * wavelet.view(wavelet.shape[0], 1, 1, wavelet.shape[-1])
...
def _physics_loss_terms(
    ...
    if energy_weight != 0.0:
        energy = energy_stability_loss(pred_phys)
    ...
    if receiver_weight != 0.0:
        ...
        receiver = receiver_waveform_loss(
            pred_phys,
            target_phys,
...
def _initialize_model_from_checkpoint(checkpoint_path: str | Path, model: torch.nn.Module, device: torch.device) -> str:
    ...
    try:
        ...
        # Warm-start tolerance: only allow parameters ABSENT from the parent
        # (new zero-init branches such as softmax gates). Never allow the
        # parent to carry tensors the child dropped, or shape mismatches.
        if unexpected:
            raise RuntimeError(
                f"init checkpoint carries tensors the model does not have: {sorted(unexpected)[:8]}"
            ) from strict_error
        if not missing:
            raise
        incompatible = [
            key for key in (model_keys & state_keys)
            if tuple(model.state_dict()[key].shape) != tuple(state[key].shape)
        ]
        if incompatible:
            raise RuntimeError(
                f"init checkpoint shape mismatch on: {sorted(incompatible)[:8]}"
            ) from strict_error
...
def _build_distillation_teacher(
    ...
    teacher = AcousticFNO3D(**teacher_config)
...
def build_training_model(config: dict[str, Any]) -> tuple[torch.nn.Module, dict[str, Any]]:
    ...
    if model_name in {"uno", "acoustic_uno", "u_no", "ufno"}:
        from .model_uno import AcousticUNO3D

        return AcousticUNO3D(**_model_kwargs(model_cfg)), model_cfg
    if model_name in {"neuralop_fno", "neuralop_tfno", "neuralop_uno"}:
        from .model_neuralop import (
            build_neuralop_fno,
            build_neuralop_tfno,
            build_neuralop_uno,
        )
        ...
        builder = {
            "neuralop_fno": build_neuralop_fno,
            "neuralop_tfno": build_neuralop_tfno,
            "neuralop_uno": build_neuralop_uno,
        }[model_name]
        return builder(**_model_kwargs(model_cfg)), model_cfg
    if model_name in {"acoustic_fno", "acousticfno3d", "fno"}:
        return AcousticFNO3D(**_model_kwargs(model_cfg)), model_cfg
...
def run_training(
    ...
    ddp = _ddp_enabled(config)
    ddp_rank, ddp_world = _ddp_rank_world_size()
    is_rank0 = not ddp or ddp_rank == 0
    if ddp:
        if not dist.is_initialized():
            dist.init_process_group(backend="nccl")
        torch.cuda.set_device(ddp_rank)
    set_seed(int(config.get("seed", 2026)))
    device = torch.device(f"cuda:{ddp_rank}") if ddp else choose_device(config["train"].get("device", "auto"))
    splits = _load_or_create_splits(config)
    if is_rank0:
        _write_split_snapshot(config, splits)
    stats = _load_or_compute_normalization(config, splits["train"], write=is_rank0)
    if ddp and _adaptive_sampling_enabled(config):
        raise ValueError("adaptive sampling is not supported with train.ddp")
    val_indices, validation_selection = _select_validation_indices(config, splits["val"])

    if ddp:
        val_loader = _loader(config, val_indices[ddp_rank::ddp_world], stats, shuffle=False)
    else:
    ...
    with metrics_path.open(metrics_mode, encoding="utf-8", newline="") as f:
        ...
        for epoch in range(start_epoch, total_epochs):
            ...
            if ddp:
                epoch_train_indices = _epoch_shards(
                    epoch_train_indices,
                    shard_count=ddp_world,
                    epoch=epoch,
                    shuffle=_train_shuffle_enabled(config),
                    seed=int(config.get("seed", 2026)),
                )[ddp_rank]
                ...
                for batch in train_batches:
                    ...
                    with torch.cuda.amp.autocast(enabled=use_amp):
                        ...
                        if _spatial_importance_sampling_enabled(config):
                            ...
                            physics_loss, _ = _physics_loss_terms(pred, y, batch, stats, config["loss"])
                            loss = loss + physics_loss
                        else:
                            loss, _ = combined_loss(pred, y, **_supervised_loss_config(config["loss"]))
                            physics_loss, _ = _physics_loss_terms(pred, y, batch, stats, config["loss"])
                            loss = loss + physics_loss
                    ...
                    if use_scaler:
                        ...
                        loss.backward()
                        if ddp:
                            _allreduce_grads_complex_safe(model, ddp_world)
                        if config["train"].get("grad_clip") is not None:
                            torch.nn.utils.clip_grad_norm_(model.parameters(), float(config["train"]["grad_clip"]))
                        optimizer.step()
                    global_step += 1
                    ...
                    if is_rank0 and _should_log_train_step(global_step, log_every_steps):
                        print(
            ...
            if ddp:
                sync_bucket = [best_val, epochs_without_improvement, stop_training]
                dist.broadcast_object_list(sync_bucket, src=0)
                best_val, epochs_without_improvement, stop_training = sync_bucket
            if (
            ...
            if is_rank0:
                ...
                f.flush()
            if is_rank0 and _adaptive_sampling_enabled(config):
                write_json(residual_ema_path, residual_ema)
            if is_rank0 and _spatial_importance_sampling_enabled(config) and spatial_residual_ema is not None:
                torch.save(
                    {
            ...
            if is_rank0 and _should_save_epoch_checkpoint(epoch, total_epochs, checkpoint_every_epochs):
                save_checkpoint(last_path, payload)
            if is_rank0 and improved:
    ...
    if ddp:
        result["ddp_rank"] = ddp_rank
        result["ddp_world_size"] = ddp_world
    if output_metrics:
        if is_rank0:
            write_json(output_metrics, result)
        else:
            result = {"ddp_rank": ddp_rank, "status": "worker_complete"}
    if ddp:
